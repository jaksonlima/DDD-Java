...
public class VideoValidator extends Validator {
    ...
    private void checkTitleConstraints() {
        ...
        if (title == null) {
            this.validationHandler().append(new Errors("'title' should not be null"));
            return;
        }

        if (title.isBlank()) {
            this.validationHandler().append(new Errors("'title' should not be empty"));
            return;
        }

        final int length = title.trim().length();
        if (length > TITLE_MAX_LENGTH) {
            this.validationHandler().append(new Errors("'title' must be between 1 and 255 characters"));
        }
    }
    ...
    private void checkDescriptionConstraints() {
        ...
        if (description == null) {
            this.validationHandler().append(new Errors("'description' should not be null"));
            return;
        }

        if (description.isBlank()) {
            this.validationHandler().append(new Errors("'description' should not be empty"));
            return;
        }

        final int length = description.trim().length();
        if (length > DESCRIPTION_MAX_LENGTH) {
            this.validationHandler().append(new Errors("'description' must be between 1 and 4000 characters"));
        }
    }

    private void checkLaunchedAtConstraints() {
        if (this.video.getLaunchedAt() == null) {
            this.validationHandler().append(new Errors("'launchedAt' should not be null"));
        }
    }

    private void checkRatingConstraints() {
        if (this.video.getRating() == null) {
            this.validationHandler().append(new Errors("'rating' should not be null"));
        }
    }
}